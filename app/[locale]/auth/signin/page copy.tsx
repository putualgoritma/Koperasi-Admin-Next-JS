...
const SignIn: React.FC = () => {
  ...
  //   const auth = await getServerAuthSession(); //(1) 
  //   // setAuthSession(auth);
  //   // return 
  //   console.log('iniiiiis ',auth);
  // }


  // useEffect(()=>{
  // getSn()
  //   console.log('jkjk : ', authSession)
  // },[])
   ...
   async function login(e:SyntheticEvent){
    ...
    await fetch('http://127.0.0.1:8000/api/v1/open/users/login',{
    method : 'POST',
    headers : {
...
.then((data) => {
  if(data.success){
  console.log('inni dari laravel ', data)
  console.log('inni dari laravel2 ', data.user, data.permissions, data.success.token)
  // setToken(data.user,data.permissions, data.success.token)

  const datas = {
    user :  data.user,
    role : data.permissions,
    token : 'Bearer '+data.success.token
  }
  console.log('iniii dat : ', data.permissions)
  //     const data = {
  //   user : 'ab@email.com',
  //   role : ['user_access','user_create','user_delete','user_update']
  // }
  var dataToJson =  JSON.stringify(data)
  var ciphertext = CryptoJS.AES.encrypt(dataToJson, 'secret key k01in').toString()
  
  // console.log('iniii dat : ', ciphertext)
  Cookies.set('__d__', ciphertext)
  ...
  function setToken(user : any, permissions : any, token : string){
    const datas = {
      user : user,
      role : permissions,
      token : 'Bearer '+token
    ...
    //   role : ['user_access','user_create','user_delete','user_update']
    // }
    var dataToJson =  JSON.stringify(datas)
    var ciphertext = CryptoJS.AES.encrypt(dataToJson, 'secret key k01in').toString()
