                        ...
                        <form onSubmit={handleSubmit} className="group">
                            ...
                            <div className={`overflow-y-scroll h-75 custom-scrollbar ${tab === 1 ? "" : "hidden"}`}>
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{intl.user.contacts.email}</label>
                                    <input
                                        value={data.email}
                                ...
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{intl.user.contacts.password} <span className="text-meta-1">*</span></label>
                                    <input
                                        value={data.password}
                                ...
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{intl.user.contacts.c_password} <span className="text-meta-1">*</span></label>
                                    <input
                                        ref={cPasswordRef}
                                ...
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{intl.user.contacts.register}</label>
                                    <input
                                        value={data.register}
                                ...
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{intl.user.contacts.type}</label>
                                    <select onChange={(e) => setData({ ...data, type: e.target.value })} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input" required>
                                        <option disabled selected >{intl.user.contacts.type}</option>
                                        {data.type == 'member' ?
                                            <option value={"member"} disabled selected>{intl.user.contacts.member}</option>
                                            :
                                            <option value={"member"}>{intl.user.contacts.member}</option>
                                        }
                                        {data.type == 'staff' ?
                                            <option value={"staff"} disabled selected>{intl.user.contacts.staff}</option>
                                            :
                                            <option value={"staff"}>{intl.user.contacts.staff}</option>
                                        }
                                        {data.type == 'admin' ?
                                            <option value={"admin"} disabled selected>{intl.user.contacts.admin}</option>
                                            :
                                            <option value={"admin"}>{intl.user.contacts.admin}</option>
                                        }
                                    </select>
                                </div>
                            ...
                            <div className={`overflow-y-scroll h-75 custom-scrollbar ${tab === 2 ? "" : "hidden"}`}>
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{intl.user.contacts.contact_id}</label>
