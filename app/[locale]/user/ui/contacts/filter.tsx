                            ...
                            <form onSubmit={handleSubmit}>
                                ...
                                <div className="form-control">
                                    ...
                                    <select onChange={(e) => handleForm('status', e.target.value)} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input">
                                        <option value='' selected={(form.status == '') ? 'selected' : ''}>{messagesIntl.user.contacts.status}</option>
                                ...
                                <div className="form-control">
                                    ...
                                    <select onChange={(e) => handleForm('type', e.target.value)} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input">
                                        <option value='' selected={(form.type == '') ? 'selected' : ''}>{messagesIntl.user.contacts.type}</option>
                                        <option value='member' selected={(form.type == 'member') ? 'selected' : ''}>{messagesIntl.user.contacts.type_option.member}</option>
                                        <option value='staff' selected={(form.type == 'staff') ? 'selected' : ''}>{messagesIntl.user.contacts.type_option.staff}</option>
                                        <option value='admin' selected={(form.type == 'admin') ? 'selected' : ''}>{messagesIntl.user.contacts.type_option.admin}</option>
                                    </select>
                                </div>
                                ...
                                <div className="flex flex-row gap-2">
                                    <select onChange={(e) => handleForm('order_by', e.target.value)} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input">
                                        ...
                                        <option value='email' selected={(form.order_by == 'email') ? 'selected' : ''}>{messagesIntl.user.contacts.email}</option>
                                        <option value='code' selected={(form.order_by == 'code') ? 'selected' : ''}>{messagesIntl.user.contacts.code}</option>
                                        <option value='register' selected={(form.order_by == 'register') ? 'selected' : ''}>{messagesIntl.user.contacts.register}</option>
                                        <option value='status' selected={(form.order_by == 'status') ? 'selected' : ''}>{messagesIntl.user.contacts.status}</option>
                                        <option value='type' selected={(form.order_by == 'type') ? 'selected' : ''}>{messagesIntl.user.contacts.type}</option>
                                    </select>
